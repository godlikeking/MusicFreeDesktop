...
> 🍴 **本 Fork 额外提供 Windows x64 便携版**（免安装，用户数据随程序目录 `portable/` 移动），详见 [Releases](https://github.com/godlikeking/MusicFreeDesktop/releases)。
...
| **🔀 切换来源** | 同一首歌可在不同音源插件之间一键换源，正在播放时无缝切换并保持进度，关联关系持久化。详见下方 [切换来源](#-切换来源) 章节。                                               |
...
同一首歌在不同音源插件上可能存在音质差异、版权限制或失效情况。本 Fork 新增**单曲切换来源**：把某首歌的播放地址切换到其他插件上的同名歌曲，无需重新搜索或修改歌单。

### 使用方法

1. 在任意歌曲列表（搜索结果、歌单、专辑、本地音乐、播放队列等）中，**右键单击歌曲**；
2. 选择 **「切换来源」**（多选时不显示，仅支持单曲）；
3. 在弹窗中选择目标音源插件的 Tab，关键词已自动填为「歌名 歌手」，可手动修改后重新搜索；
4. 点击目标歌曲即完成换源，当前已关联的来源会显示 ✓ 标记。

### 行为说明

- **正在播放时**：立即无缝切换到新音源，保持当前播放进度与播放/暂停状态；非当前歌曲则在下次播放时生效；
- **持久化**：关联关系保存在本地数据库，重启应用后自动沿用；
- **来源标签同步**：换源后，底部播放栏、播放队列、歌曲列表的「来源」列以及全屏播放器都会显示新音源；
- **歌曲身份不变**：歌单、收藏、歌词、下载等仍归属原平台歌曲记录，仅播放流来自新音源；
- **自动回退**：当新音源的所有音质均无法取流时，自动回退到原平台播放。

> 与插件级「音源重定向」（在插件管理中配置，对整个插件生效）不同，切换来源是**针对单首歌曲**的设置。
